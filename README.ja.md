...
- `--query-preprocess` は、検索前にクエリをどのようにトークン化するかを制御します:
  ...
  - `auto`(デフォルト)— インデックスのアナライザーでクエリをトークン化し、トークンから検索クエリを直接構築して AND で結合します。CJK トークンには単語境界を保つための文字単位フレーズのフォールバックが付きます。クエリ文字列はクエリパーサーを経由しないため、Tantivy のクエリ構文(`AND`、`OR`、`NOT` などの予約語や特殊文字)はリテラルとして扱われます。
